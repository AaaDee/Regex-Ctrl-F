...
Tällä hetkellä sovellus löytää tekstistä vain ensimmäisen osuman, vaikka alussa tarkoitus oli, että kaikki osumat voisi selata läpi. Tämä jäi toteuttamatta lähinnä ajanpuutteen vuoksi. Itse algoritmin puolella tämä olisi melko triviaali lisäys (aloita simulointi tietystä kohdasta tekstiä ja päivitä laskurit), mutta käyttöliittymäpuolella työtä olisi ollut hieman enemmän (milloin "Etsi seuraava" -nappi on näkyvissä, mitä tapahtuu jos tiedosto vaihtuu jne.), mikä ei valitettavasti ehtinyt valmiiksi viimeiseen palautukseen.

Toinen ilmeinen kehityssuunta on tulkin ymmärtämien toimenpiteiden laajentaminen. Periaatteessa nykyisillä erikoismerkeillä voitaisiin toisintaa valtaosa säännöllisten lausekkeiden yleisesti käytössä olevista erikoismerkeistä, mutta tämä on usein melko työlästä. Lisäksi erillinen pakomerkki ("\" tai vastaava) olisi hyvä toteuttaa.
